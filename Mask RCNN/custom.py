...
COCO_WEIGHTS_PATH = os.path.join(ROOT_DIR, "drive/My Drive/input/mask_rcnn_coco.h5")
...
class CustomConfig(Config):
    ...
    NUM_CLASSES = 4 + 1  # Car Background + damages
...
class CustomDataset(utils.Dataset):
    def load_custom(self, dataset_dir, subset):
        ...
        self.add_class("damage", 1, "scratch") # 스크래치
        self.add_class("damage", 2, "dent") # 찌그러짐
        self.add_class("damage", 3, "spacing") # 이격
        self.add_class("damage", 4, "clean") # 해당 없음
        ...
        for a in annotations:
            ...
            self.add_image(
                "damage",  ## for a single class just add the name here (delete class_ids)
                image_id=a['filename'],  # use file name as a unique image id
                path=image_path,
                width=width, height=height,
                polygons=polygons
                , class_ids=class_ids # multi class
                )
